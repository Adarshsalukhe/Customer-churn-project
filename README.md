...
**Requirements:** [Git](https://git-scm.com/) and [Docker](https://www.docker.com/products/docker-desktop/) installed on your machine. No Python setup needed — Docker handles all dependencies.

**1. Clone the repo**

```bash
git clone https://github.com/Adarshsalukhe/Customer-churn-project.git
...
Go to `http://127.0.0.1:8000/docs` in your browser. This is FastAPI's auto-generated Swagger UI — expand the `POST /predict` endpoint, click "Try it out," paste in customer data (example below), and click "Execute" to get a live prediction. No code required.
...
response = requests.post(
    ...
    json={
        ...
        "PaymentMethod": "Electronic check", "MonthlyCharges": 85.5, "TotalCharges": 450.0
    }
)
print(response.json())
```
...
This currently runs locally only — there's no hosted public URL, so users need Docker installed and the container running on their own machine to use the API.
